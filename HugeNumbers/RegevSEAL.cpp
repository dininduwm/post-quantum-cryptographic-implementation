...
void genarateKeys(privateKey *private_key, publicKey *public_key)
{
    ...
    matMulAddLarge(private_key->sT, public_key->A, eT, public_key->bT, 1, n, m, q);
...
cipherText encrypt(publicKey public_key, dtype message_bit[numberBits])
{
    ...
    matMulLarge(public_key.A, x, cipher_text.u, n, m, numberBits, q);
    ...
    // bTx = public_key.bT * X;
    matMulLarge(public_key.bT, x, bTx, 1, m, numberBits, q);
...
dtype *decrypt(privateKey private_key, cipherText cipher_text)
{
    ...
    matMulLarge(private_key.sT, cipher_text.u, sTu, 1, n, numberBits, q);
